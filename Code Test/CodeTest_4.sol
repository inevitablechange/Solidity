...
*/
    ...
    function setUser(string memory name) public {
        uint number = users.length + 1;
        address walletAddress = msg.sender;
        uint score = 0;
        uint balance = 0;

        users.push(user(number,name,walletAddress,score,balance));
    }

    // 유저 조회 기능 - 유저의 전체정보 번호, 이름, 주소, 점수를 반환. 
    function getUser(uint number) public view returns(user memory) {
        return users[number-1];
    }

    // 게임 참가시 참가비 제출 기능 - 참가시 0.01eth 지불 (돈은 smart contract가 보관)
    function payForGame() public payable {
        user memory joinUser;
        require(msg.value == 0.01 ether, "pay 0.01 eth");
        for(uint i=0; i<users.length;i++) {
            if(msg.sender == users[i].walletAddress) {
                joinUser = users[i];
            }
        }

        for(uint i=0; i<gameUsers.length;i++) {
            if(users[i].number == 0) {
                users[i] = joinUser;
                if(i == gameUsers.length-1) {
                    for(uint j=0;j<gameUsers.length;j++) {
                        for(uint k=0;k<users.length;k++) {
                            if(gameUsers[j].number == users[k].number) {
                                users[k].score += (4 - j);
                            }
                        }
                    }
    ...
    // 점수를 돈으로 바꾸는 기능 - 10점마다 0.1eth로 환전
    function changeScoreToEth() public {
        for(uint i=0; i<users.length;i++) {
            if(msg.sender == users[i].walletAddress) {
                require(users[i].score >= 10 , "need 10 points");
                users[i].score -= 10;
    ...
    // 관리자 인출 기능 - 관리자는 0번지갑으로 배포와 동시에 설정해주세요, 관리자는 원할 때 전액 혹은 일부 금액을 인출할 수 있음 (따로 구현)
    function withdraw(uint etherAmount) public {
        require(msg.sender == admin, "only admin");
        payable(admin).transfer(etherAmount);
    }
